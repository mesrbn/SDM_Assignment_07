...
/**
 * Bitmap indexes build bitmaps for distinct values of a column and allow to retrieve the rowIds for a lookup expression
 * What and how bitmaps are created are determined by the subclasses.
 * @author melhindi
 *
 * @param <T>
 */
public abstract class AbstractBitmapIndex<T extends AbstractSQLValue> extends AbstractUniqueIndex<T>{
	...
	public AbstractBitmapIndex(AbstractTable table, int keyColumnNumber) {
		...
		if (this.getTable().getRecordCount() == 0) {
			System.out.println("WARNING: Initializing a Bitmap index with an empty table does not make sense! This Bitmap index is a static index!");
		}
	...
	/**
	 * Reads the table of the BitmapIndex instance and initializes any required bitmaps (member variable bitMaps) for the index's
	 * key column (member variable keyColumnNumber) ) according to a naive bitmap approach.
	 * 
	 */
	 ...
	 * StartKey and endKey are AbstractSQLValue instances that specify the lower- and upper bound of a range.
	 * A record falls into the range if startKey <= recordKey <= endKey.
	 * @param startKey AbstractSQLValue instance that specifies the lower bound (inclusive) of the range to lookup.
	 * @param endKey AbstractSQLValue instance that specifies the upper bound (inclusive) of the range to lookup.
	 * @return List of Abstract Records stored in the indexed table and that fall into the specified range.
	 *         Returns an empty list if no records fall into the range.
	 */
